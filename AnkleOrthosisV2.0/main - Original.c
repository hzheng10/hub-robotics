...
int16_t main(void) {
    ...
    while (1) {
        ...
        // Read analog signal
        u= Samp2Voltage(ReadAnalogSample(0)); //read voltage from channel 0 (AN2) PIN2
        // Filtered
        heel_sensor = 0.945*heel_sensor+0.0549*u;
        knee_angle = Samp2Voltage(ReadAnalogSample(1));  //read voltage froom channel 1 (AN3) PIN3
        force_load_cell = Samp2Voltage(ReadAnalogSample(4));    // read voltage from channel 4 (AN4) PIN6
        force_load_cell = 3.03*force_load_cell-5;       // from 0 to 3.3 volt to -5 to 5 volt.
        force_load_cell = 100*force_load_cell*4.448+20; // convert -5 to 5 volt to force in N.


        knee_angle = KneeAngle(knee_angle); //calculate the knee_angle from the potentiometer volage
        ...
        else 
        {
            switch (state)
            {
                ...
                case ST_EARLY_STANCE:
                    ...
                    if (count>=1680) // delay 0.86 sec  (2000 for 1 sec)
                    {
                        state = ST_PRE_SWING;
                        count=0;
                        break;
                    ...
                    count+=1;
                    // test knee_angle
                    test = desired_force/1000;
                    setVoltage(0,Voltage2Samp(test));
